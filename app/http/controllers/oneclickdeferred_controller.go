...
type OneclickDeferredController struct {
	inscription *oneclick.MallInscription
	transaction *oneclick.MallTransaction
}

func NewOneclickDeferredController() (*OneclickDeferredController, error) {
	options := transbank.Options{
		Environment:  transbank.Integration,
		CommerceCode: "597055555547",
	...
	return &OneclickDeferredController{
		inscription: inscription,
		transaction: transaction,
	}, nil
}

func (o *OneclickDeferredController) Index(ctx http.Context) http.Response {
	...
	data := map[string]any{
		...
		"responseUrl":     fullUrl,
		"isDeferred":      true,
		"codigoComercio1": 597055555548,
		"codigoComercio2": 597055555549,
	}
	return ctx.Response().View().Make("oneclick/index.tmpl", data)
}

func (o *OneclickDeferredController) StartInscription(ctx http.Context) http.Response {
...
func (o *OneclickDeferredController) FinishInscription(ctx http.Context) http.Response {
...
func (o *OneclickDeferredController) DeleteInscription(ctx http.Context) http.Response {
	request := ctx.Request().All()
...
func (o *OneclickDeferredController) AuthorizeTransaction(ctx http.Context) http.Response {
	request := ctx.Request().All()
...
func (o *OneclickDeferredController) StatusTransaction(ctx http.Context) http.Response {
	request := ctx.Request().All()
	buyOrder := ctx.Request().Input("buy_order")
...
func (o *OneclickDeferredController) RefundTransaction(ctx http.Context) http.Response {
	request := ctx.Request().All()
	buyOrder := ctx.Request().Input("buy_order")
...
func (o *OneclickDeferredController) CaptureTransaction(ctx http.Context) http.Response {
	request := ctx.Request().All()
	commerceCode := ctx.Request().Input("child_commerce_code")
	buyOrder := ctx.Request().Input("buy_order")
	authCode := ctx.Request().Input("authorization_code")
	captureAmountStr := ctx.Request().Input("capture_amount")
	captureAmount, err := strconv.ParseFloat(captureAmountStr, 64)
	...
	response, err := o.transaction.Capture(buyOrder, commerceCode, authCode, captureAmount)
	if err != nil {
		return ctx.Response().Status(500).Json(map[string]any{
			"error": err,
		})
	...
	data := map[string]any{
		"title":    "Transacción Capturada",
		"request":  request,
		"response": response,
	}

	return ctx.Response().View().Make("oneclick/malltransaction_capture.tmpl", data)
}
