...
type MallTransactionController struct {
	mallTransaction *webpayplus.MallTransaction
}

func NewMallTransactionController() (*MallTransactionController, error) {
	options := transbank.Options{
		Environment:  transbank.Integration,
		CommerceCode: "597055555535",
		ApiKey:       "579B532A7440BB0C9079DED94D31EA1615BACEB56610332264630D42D0A36B1C",
	}
	transaction, err := webpayplus.NewMallTransaction(options)
	if err != nil {
		return nil, err
	}
...
func (m *MallTransactionController) Index(ctx http.Context) http.Response {
	...
	data := map[string]any{
		...
		"returnUrl":     fullUrl,
		"isDeferred":    false,
		"commerceCode1": 597055555536,
		"commerceCode2": 597055555537,
	}
...
func (m *MallTransactionController) CreatedTransaction(ctx http.Context) http.Response {
	...
	var details []webpayplus.MallDetails
	for i := 0; ; i++ {
		...
		details = append(details, webpayplus.MallDetails{
			Amount:       amount,
...
func (m *MallTransactionController) CommitedTransaction(ctx http.Context) http.Response {
	...
	data := map[string]any{
		"title":      "Transacción Mall Confirmada",
		"request":    request,
		"response":   response,
...
func (m *MallTransactionController) GetTransactionStatus(ctx http.Context) http.Response {
	request := ctx.Request().All()
	...
	data := map[string]any{
		"title":    "Status de Transacción Mall",
		"request":  request,
		"response": response,
	}
...
func (m *MallTransactionController) RefundTransaction(ctx http.Context) http.Response {
	request := ctx.Request().All()
	...
	data := map[string]any{
		"title":    "Transacción Mall Reembolsada",
		"request":  request,
		"response": response,
	}
